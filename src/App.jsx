...
export default function App(){
  ...
  // Dormant: reviewed at least once, timer still running (nextReviewAt in future)
  const dormantCards=useMemo(()=>ALL_CARDS.filter(c=>{const cl=getLevel(cardLevels,c.id);return cl.level>1&&cl.nextReviewAt&&new Date(cl.nextReviewAt).getTime()>(Date.now()+timeOffset);}),[cardLevels,timeOffset]);
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bgGrad,fontFamily:"'Outfit',sans-serif",color:T.text,display:"flex",flexDirection:"column",alignItems:"center",padding:"14px 14px 80px",boxSizing:"border-box",transition:"background .4s"}}>
      ...
      {showList&&<div style={{position:"fixed",inset:0,zIndex:100,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,.4)",padding:20}} onClick={()=>setShowList(null)}><div onClick={e=>e.stopPropagation()} style={{background:T.overlayBg,borderRadius:24,padding:"24px 20px",width:"100%",maxWidth:440,maxHeight:"85vh",overflowY:"auto",boxShadow:"0 20px 60px rgba(0,0,0,.2)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><h2 style={{fontSize:20,fontWeight:800,color:T.text,margin:0}}>{showList==="known"?`⭐ ${u.mastered}`:showList==="dormant"?`💤 ${u.dormant}`:`📖 ${u.learning}`} ({(showList==="known"?knownSet.size:showList==="dormant"?dormantCards.length:learningSet.size)})</h2><button onClick={()=>setShowList(null)} style={{width:34,height:34,borderRadius:"50%",border:`1px solid ${T.pillBd}`,background:T.btnBg,color:T.btnTx,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button></div>
        {showList==="dormant"&&<div style={{fontSize:12,color:T.muted,marginBottom:10,padding:"8px 12px",borderRadius:10,background:`${T.accent}08`,border:`1px solid ${T.accent}15`,lineHeight:1.4}}>{lang==="de"?"Karten in ihrer Wartezeit. Tippe eine Karte an, um sie anzusehen. Wenn der Timer abläuft, werden sie automatisch in die obere 'Fällig'-Liste verschoben.":"Cards in their waiting period. Tap a card to view it. When each timer expires, they'll move to the top 'Due' list automatically."}</div>}
        {showList==="dormant"
          ?dormantCards.slice().sort((a,b)=>new Date(getLevel(cardLevels,a.id).nextReviewAt).getTime()-new Date(getLevel(cardLevels,b.id).nextReviewAt).getTime()).map(c=>{const lv=getLevel(cardLevels,c.id);const front=lang==="de"?(c.frontDe||c.front):c.front;return(<button key={c.id} onClick={()=>jumpToCard(c.id)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"11px 14px",borderRadius:14,border:`1px solid ${T.pillBd}`,background:T.pillBg,marginBottom:6,cursor:"pointer",textAlign:"left",fontFamily:"inherit"}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:700,color:T.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{front}</div><div style={{fontSize:13,color:CC[c.cat],fontWeight:600,marginTop:1}}>{c.back} · {c.tl}</div></div><div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:3,flexShrink:0,marginLeft:8}}><span style={{fontSize:10,fontWeight:700,color:LVL_C[lv.level-1],background:`${LVL_C[lv.level-1]}15`,padding:"2px 7px",borderRadius:6}}>Lv{lv.level}</span><span style={{fontSize:11,fontWeight:700,color:"#7B8FA1",background:"rgba(123,143,161,.12)",padding:"2px 7px",borderRadius:6}}>⏳ {fmtRelTime(lv.nextReviewAt,timeOffset)}</span></div></button>);})
        ...
        {showList==="dormant"&&dormantCards.length===0&&<div style={{textAlign:"center",padding:"30px 20px",color:T.muted,fontSize:14}}>{lang==="de"?"Keine ruhenden Karten. Alle Karten sind aktiv oder noch nicht geübt.":"No dormant cards. All cards are either active or not yet practiced."}</div>}
      </div></div>}
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",maxWidth:560}}>
        ...
        {tab==="settings"&&<div>
          ...
          <div style={{padding:"14px",borderRadius:16,background:T.pillBg,border:`1px solid ${T.pillBd}`,marginBottom:8,boxShadow:T.catCardShadow}}>
            <div style={{fontSize:12,fontWeight:600,color:T.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:6}}>{lang==="de"?"Dein Name":"Your name"}</div>
            <input type="text" value={userName} onChange={e=>setUserName(e.target.value)} placeholder={lang==="de"?"Name eingeben":"Enter your name"} style={{width:"100%",padding:"10px 12px",borderRadius:12,border:`1.5px solid ${T.inputBd}`,background:T.inputBg,color:T.text,fontSize:15,fontFamily:"'Outfit',sans-serif",fontWeight:600,outline:"none",boxSizing:"border-box"}}/>
            <div style={{fontSize:11,color:T.muted,marginTop:6}}>{user.email}</div>
          </div>

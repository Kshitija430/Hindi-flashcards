  ...
  return(
    <div style={{minHeight:"100vh",background:T.bgGrad,fontFamily:"'Outfit',sans-serif",color:T.text,display:"flex",flexDirection:"column",alignItems:"center",padding:"14px 14px 80px",boxSizing:"border-box",transition:"background .4s"}}>
      ...
      <div style={{position:"relative",zIndex:1,width:"100%",maxWidth:560}}>
        {tab==="practice"&&<>
          <div style={{marginBottom:8,display:"flex",alignItems:"center",gap:10}}><div style={{flexShrink:0}}><div style={{fontSize:11,letterSpacing:3,color:T.accent,fontWeight:700,marginBottom:1,fontFamily:"'Noto Sans Devanagari',sans-serif"}}>हिन्दी सीखें</div><h1 style={{fontSize:20,fontWeight:800,margin:0,color:T.text,whiteSpace:"nowrap"}}>Namaste, {displayName}!</h1></div>{!inCardView&&<div style={{flex:1,minWidth:0}}><input type="text" placeholder={`🔍 ${u.search}`} value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} style={{width:"100%",padding:"10px 14px",borderRadius:14,border:`1.5px solid ${T.inputBd}`,background:T.inputBg,color:T.text,fontSize:14,fontFamily:"'Outfit',sans-serif",outline:"none",boxSizing:"border-box"}}/></div>}</div>
          ...
          {!inCardView&&<>
            ...
            <div style={{display:"flex",gap:8,marginBottom:8}}>
              <button onClick={()=>enterCategory("All")} style={{flex:1,padding:"14px 12px",borderRadius:16,border:`1.5px solid ${T.accent}30`,background:`${T.accent}08`,cursor:"pointer",fontFamily:"inherit",display:"flex",flexDirection:"column",alignItems:"center",gap:3,boxShadow:T.catCardShadow}}>
              ...
              {dueCards.length>0&&<button onClick={()=>{setPracticeMode("due");setShuffledCards(shuffleArr(dueCards));setShuffled(true);setIdx(0);}} style={{flex:1,padding:"14px 12px",borderRadius:16,border:"1.5px solid #D0606030",background:"#D0606008",cursor:"pointer",fontFamily:"inherit",display:"flex",flexDirection:"column",alignItems:"center",gap:3,boxShadow:T.catCardShadow}}>
                <span style={{fontSize:22}}>🔥</span>
            ...
            {/* ——— Favourites section ——— */}
            {favCards.length>0&&<button onClick={()=>{setActiveCategory("__fav__");setIdx(0);setFlipped(false);setShowHint(false);const fc=ALL_CARDS.filter(c=>favorites.has(c.id));setShuffledCards(shuffleArr(fc));setShuffled(true);}} style={{width:"100%",padding:"12px",borderRadius:16,border:"1.5px solid #E2555530",background:"#E2555508",cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:10,marginBottom:8,boxShadow:T.catCardShadow}}>
            ...
            <div style={{fontSize:14,fontWeight:700,color:T.muted,textTransform:"uppercase",letterSpacing:1.5,marginBottom:6,padding:"0 2px"}}>{u.cats}</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,marginBottom:10}}>
              {CATEGORIES.filter(c=>c!=="All").map(cat=>{const cc=CC[cat]||T.accent;const catCards=ALL_CARDS.filter(c=>c.cat===cat);const mastered=catCards.filter(c=>getLevel(cardLevels,c.id).level>=5).length;const due=catCards.filter(c=>isDue(cardLevels,c.id)).length;const pctCat=catCards.length?Math.round((mastered/catCards.length)*100):0;return(<button key={cat} onClick={()=>enterCategory(cat)} style={{padding:"12px 12px",borderRadius:16,border:`1px solid ${T.catCardBd}`,background:T.catCardBg,boxShadow:T.catCardShadow,cursor:"pointer",textAlign:"left",fontFamily:"inherit",display:"flex",flexDirection:"column",gap:4,position:"relative",overflow:"hidden"}}><div style={{position:"absolute",top:0,left:0,right:0,height:3,background:cc}}/><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:22}}>{CAT_EMOJI[cat]||"📁"}</span><span style={{fontSize:11,fontWeight:600,color:T.muted}}>{catCards.length}</span></div><div style={{fontSize:16,fontWeight:700,color:T.text}}>{catName(cat)}</div><div style={{height:4,borderRadius:2,background:T.dotBg,overflow:"hidden"}}><div style={{height:"100%",borderRadius:2,width:`${pctCat}%`,background:cc}}/></div><div style={{display:"flex",justifyContent:"space-between",fontSize:12,color:T.muted}}><span>{mastered}/{catCards.length} ⭐</span>{due>0&&<span style={{color:"#D06060",fontWeight:600}}>{due} {u.dueNow}</span>}</div></button>);})}
            </div>

            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8,padding:"10px 0",borderTop:`1px solid ${T.divider}`}}>{[{l:u.fav,v:favCards.length,c:"#E25555",i:"❤️",cl:()=>{if(favCards.length){setActiveCategory("__fav__");setIdx(0);setFlipped(false);setShowHint(false);setShuffledCards(shuffleArr(favCards));setShuffled(true);}}},{l:lang==="de"?"Am Lernen":"In progress",v:learningSet.size,c:"#C8A040",i:"📖",cl:()=>setShowList("learning")},{l:lang==="de"?"Gemeistert":"Nailed",v:knownSet.size,c:"#40A050",i:"⭐",cl:()=>setShowList("known")},{l:u.due,v:dueCards.length,c:"#D06060",i:"🔥",cl:()=>{setPracticeMode("due");setShuffledCards(shuffleArr(dueCards));setShuffled(true);setIdx(0);}}].map(s=>(<div key={s.l} onClick={s.cl} style={{textAlign:"center",padding:"10px 4px",borderRadius:14,background:T.pillBg,border:`1px solid ${T.pillBd}`,cursor:"pointer",boxShadow:T.catCardShadow,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><span style={{fontSize:16}}>{s.i}</span><span style={{fontSize:18,fontWeight:800,color:s.c}}>{s.v}</span><span style={{fontSize:11,color:T.muted,fontWeight:600}}>{s.l}</span></div>))}</div>
            </>}
          </>}
